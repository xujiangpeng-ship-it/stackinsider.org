...
# ============ 已发布文章索引 ============
# 旧实现只用「关键词转 slug 后与文件名的精确匹配」判断，命中率常年为 0，
# 于是同一产品被反复生成（X alternatives / X review and pricing / is X worth it
# 被当成三个独立关键词），产出互相 cannibalize 的近重复文章——
# 这正是 AdSense "low value content" 拒审的直接来源。现改为两层判定：
#   ① 产品实体相等：剥离「角度词」后剩下的实体 token 完全一致 -> 重复
#   ② Jaccard 相似度兜底：>= 0.70 -> 重复

# 纯语法/营销噪声词，不携带主题信息
_STOP_TOKENS = {
    'best', 'the', 'a', 'an', 'and', 'or', 'for', 'with', 'to', 'of', 'in', 'on',
    'at', 'by', 'is', 'are', 'it', 'its', 'this', 'that', 'as', 'from', 'into',
    'your', 'you', 'we', 'my', 'top', 'what', 'how', '2025', '2026',
}
# 「角度词」：同一个产品的不同切入角度。它们不改变产品实体，
# 但确实代表不同内容维度，所以从实体里剥掉后单独参与 Jaccard 兜底。
_ANGLE_TOKENS = {
    'review', 'reviews', 'pricing', 'price', 'prices', 'cost', 'costs',
    'alternatives', 'alternative', 'competitors', 'competitor',
    'comparison', 'compare', 'compared', 'comparisons', 'worth',
    'features', 'plans', 'guide', 'guides', 'pros', 'cons',
}
...
def _entity(s):
    """产品实体：剥离角度词后剩下的 token 集合。"""
    return _tokens(s) - _ANGLE_TOKENS


# 注意：必须用脚本自身所在目录做基准，不能用相对路径 + 依赖 cwd。
# 一旦 cwd 不是仓库根，listdir 会静默返回空 -> 去重彻底失效 -> 照样狂吐近重复文章。
_REPO_ROOT = os.path.dirname(os.path.abspath(__file__))
_posts_dir = os.path.join(_REPO_ROOT, "content", "posts")
_published = []          # 每项为 dict(tokens=..., entity=...)
if os.path.isdir(_posts_dir):
    for _fn in sorted(os.listdir(_posts_dir)):
        if not _fn.endswith(".md"):
            continue
        _topic = re.sub(r"^\d{4}-\d{2}-\d{2}-", "", _fn[:-3]).replace("-", " ")
        _sig = [_topic]
        # frontmatter 里的 slug / title 同样是这篇文章的主题信号
        try:
            with open(os.path.join(_posts_dir, _fn), "r", encoding="utf-8") as _f:
                _raw = _f.read(4096)
            for _k in ("slug", "title"):
                _mm = re.search(_k + r':\s*"([^"]*)"', _raw)
                if _mm:
                    _sig.append(_mm.group(1))
        except Exception:
            pass
        for _s in _sig:
            _t, _e = _tokens(_s), _entity(_s)
            if _t:
                _published.append({"tokens": _t, "entity": _e})


def _is_duplicate(kw):
    """关键词是否与某篇已发布文章同主题。三条判据任一成立即视为重复。

    阈值取保守值：漏判的代价是又产出一篇近重复文章（直接喂给
    AdSense 的 low-value 理由），误判的代价只是少发一篇——
    词库尚有数百个未用词，所以宁枉勿纵。
    """
    fp, fe = _tokens(kw), _entity(kw)
    if not fp:
        return False
    for _p in _published:
        pe = _p["entity"]
        # ① 产品实体完全一致
        if fe and pe and fe == pe:
            return True
        # ② 实体包含：候选词是已发布主题的「子集/超集」，
        #    例如 "best CRM" ⊆ "best CRM for manufacturing"，内容必然大幅重叠
        if fe and pe and (fe <= pe or pe <= fe):
            return True
        # ③ Jaccard 兜底（0.50：覆盖到产品和场景都高度接近的情形）
        _inter = len(fp & _p["tokens"])
        if _inter:
            if _inter / len(fp | _p["tokens"]) >= 0.50:
                return True
    return False
...
for _offset in range(len(keywords)):
    ...
    if not _is_duplicate(keywords[_i]):
        chosen_i = _i
        keyword = keywords[_i]
        break
else:
    # 词库已全部发过：宁可不发文，也不要再产近重复内容
    print("All %d keywords already covered - nothing new to publish." % len(keywords))
    raise SystemExit(0)
...
2. Key takeaways block (MANDATORY — this is the first thing readers and reviewers see):
   - Immediately after the YAML block, before any other content, insert EXACTLY this structure:

     ## Key takeaways
     - [First takeaway — must contain a concrete price, number, or named limitation]
     - [Second takeaway — a deciding factor for a specific team size or use case]
     - [Third takeaway — the tradeoff most buyers miss]

   - Use exactly 3 bullets (4 only if a genuinely distinct fourth point exists).
   - Each bullet must be ONE complete sentence, 12-30 words, self-contained.
   - Every bullet MUST carry a hard fact: dollar amounts, percentages, seat counts, or named product limitations.
     Weak example: "Pricing varies depending on the plan you choose."
     Strong example: "QuickBooks Online starts at $30/month for one user, but payroll costs an extra $45/month on top."
   - Do NOT use bold anywhere in this block. Do NOT end any bullet with a colon.
   - Do NOT restate the article title or open with the product name in every bullet.
...
9. Output ONLY the Markdown article from "---" to the last line. No commentary, no wrapping code blocks (```), nothing outside the article.
"""
...
# ========== Key Takeaways 守卫（AdSense low-value 防线）==========
# 模型偶尔会漏掉 takeaways 区块，或只产出 1-2 条空泛句子。
# 宁可多花一次调用补齐，也不要上线一篇开头没有摘要价值的文章。
def count_kt_bullets(text):
    m = re.search(r'^#{1,3}\s*Key takeaways\s*$(.*?)(?=^#{1,3}\s|\Z)',
                  text, re.M | re.S | re.I)
    if not m:
        return 0
    return len([b for b in re.findall(r'^\s*[-*]\s+(.+)$', m.group(1), re.M) if b.strip()])


def ensure_key_takeaways(text, kw):
    if count_kt_bullets(text) >= 3:
        return text
    print("   ⚠️ Key takeaways 缺失或不足 3 条，正在补齐...")
    body = text.split('---', 2)[2] if text.startswith('---') else text
    _p = (
        'Read this article about "%s" and write a Key Takeaways block for the top.\n\n'
        'ARTICLE:\n"""\n%s\n"""\n\n'
        'Output exactly this format and nothing else:\n\n'
        '## Key takeaways\n- takeaway one\n- takeaway two\n- takeaway three\n\n'
        'Each bullet: one sentence, 12-40 words, MUST contain a hard fact from the '
        'article (dollar price, percentage, seat count, timeline, or named limitation). '
    ...
    try:
        ...
        _b = [re.sub(r'\*\*', '', x).replace('—', ',').strip()
              for x in re.findall(r'^\s*[-*]\s+(.+)$', _raw, re.M)]
        # 丢掉可能的 "## Key takeaways" 标题行本身
        _b = [x for x in _b if not re.match(r'^#{1,3}\s', x)]
        if len(_b) >= 3:
            _block = "## Key takeaways\n" + "\n".join("- " + x for x in _b[:4]) + "\n\n"
            _parts = text.split('---', 2)
            if len(_parts) >= 3 and text.startswith('---'):
                text = "---" + _parts[1] + "---\n\n" + _block + _parts[2].lstrip('\n')
            else:
                text = _block + text
            print("   ✅ Key takeaways 已补齐 %d 条" % len(_b[:4]))
        else:
            print("   ❌ 补齐失败：模型只给了 %d 条" % len(_b))
    except Exception as _e:
        print("   ❌ 补齐异常: %s" % str(_e)[:160])
    return text


article_text = ensure_key_takeaways(article_text, keyword)
